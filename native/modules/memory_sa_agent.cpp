...
public:
    ...
    void onAttach(JavaVM* vm, jvmtiEnv* jvmti, const char* options) override {
        jvmti_ = jvmti;
        vm_ = vm;
        ...
        if (err != JVMTI_ERROR_NONE) {
            writeOutput("[Native SA] Warning: Failed to add GC capabilities: " + std::to_string(err));
        }
        ...
        writeOutput("[Native SA] Memory SA Module loaded - JVMTI Agent");
...
private:
    void parseOptions(const char* options) {
        ...
        while (pos < opts.length()) {
            ...
            if (next == std::string::npos) next = opts.length();
    ...
    void monitorMemory() {
        ...
        writeOutput("[Native SA] Starting memory analysis for " + std::to_string(duration_) + " seconds...");
    ...
    void analyzeHeapMemory(JNIEnv* env) {
        ...
        jmethodID getMemoryMXBean = env->GetStaticMethodID(memoryMXBeanClass, 
        ...
        jmethodID getHeapMemoryUsage = env->GetMethodID(memoryMXBeanInterface,
        ...
        if (getHeapMemoryUsage) {
            ...
            if (heapUsage) {
                ...
                if (getUsed && getMax && getCommitted) {
                    ...
                    oss << "[Native SA] Heap Analysis at " << std::put_time(std::localtime(&time_t), "%Y-%m-%d %H:%M:%S");
                    writeOutput(oss.str());
    ...
    void analyzeMemoryPools(JNIEnv* env) {
        ...
        jmethodID getMemoryPoolMXBeans = env->GetStaticMethodID(memoryMXBeanClass,
    ...
    void analyzeMemoryPool(JNIEnv* env, jobject pool) {
        ...
        jmethodID getUsageMethod = env->GetMethodID(poolClass, "getUsage", "()Ljava/lang/management/MemoryUsage;");
        
        if (getNameMethod && getUsageMethod) {
            jstring nameStr = (jstring)env->CallObjectMethod(pool, getNameMethod);
            ...
            if (nameStr && usage) {
                ...
                if (getUsed && getMax) {
                    ...
                    if (max > 0) {
                        ...
                        oss << " / " << formatBytes(max) << " (" << std::fixed << std::setprecision(1) << usage_percent << "%)";
                    }
                    writeOutput(oss.str());
                }
    ...
    static void JNICALL onGCStart(jvmtiEnv* jvmti) {
        ...
        auto now = std::chrono::system_clock::now();
        auto time_t = std::chrono::system_clock::to_time_t(now);
        std::cerr << "[Native SA] GC Started at " << std::put_time(std::localtime(&time_t), "%Y-%m-%d %H:%M:%S") << std::endl;
        std::cerr.flush();
    }
    ...
    static void JNICALL onGCFinish(jvmtiEnv* jvmti) {
        auto now = std::chrono::system_clock::now();
        auto time_t = std::chrono::system_clock::to_time_t(now);
        std::cerr << "[Native SA] GC Finished at " << std::put_time(std::localtime(&time_t), "%Y-%m-%d %H:%M:%S") << std::endl;
        std::cerr.flush();
    }
};

// Register module - use a safer approach
extern "C" {

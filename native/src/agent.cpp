...
void AgentManager::onAttach(JavaVM* java_vm, jvmtiEnv* jvmti, const char* options) {
    ...
    if (it != modules_.end() && it->second != nullptr) {
        try {
            protocol->sendStatus(jvmtool::StatusCode::RUNNING, "Attaching module: " + target_module);
            it->second->onAttach(java_vm, jvmti, options);
            protocol->sendStatus(jvmtool::StatusCode::SUCCESS, "Module '" + target_module + "' attached successfully");
        } catch (const std::exception& exception) {
            protocol->sendError("Module '" + target_module + "' failed to attach: " + exception.what());
        } catch (...) {
            protocol->sendError("Module '" + target_module + "' failed to attach: Unknown exception");
        }
    } else {

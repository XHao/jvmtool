...
static bool match_basename(const char* path, const char* base) {
    if (path == nullptr || base == nullptr) return false;
...
static int dl_iterate_callback(struct dl_phdr_info* info, size_t, void* data) {
    ...
    if (ctx->result != nullptr) return 0; // already found
    ...
    if (info->dlpi_name && match_basename(info->dlpi_name, ctx->target)) {
        ...
        for (int i = 0; i < info->dlpi_phnum; i++) {
            if (info->dlpi_phdr[i].p_type == PT_LOAD) {
                const unsigned char* seg_start = reinterpret_cast<const unsigned char*>(info->dlpi_addr + info->dlpi_phdr[i].p_vaddr);
        ...
        if (max_addr == nullptr) max_addr = min_addr; // fallback

        CodeCache* cache = new CodeCache(ctx->target, /*lib_index*/0, min_addr, max_addr, (const char*)info->dlpi_addr);
